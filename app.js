;(function () {
	...
	const Store = {
		async load() {
			...
			if (!res.ok) {
				throw new Error('GitHub load failed (HTTP ' + res.status + ')')
			}
		...
		async save(dataset) {
			...
			let res = await fetch(ghApiUrl(cfg), {
				method: 'PUT',
			...
			if (!res.ok) {
				...
				throw new Error(
					'GitHub save failed (HTTP ' + res.status + ') ' + detail.slice(0, 200),
				)
			}
			const j = await res.json()
			ghSha = j.content.sha
